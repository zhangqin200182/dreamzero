...
World Action Models (WAM) 的一个核心但未被充分研究的问题是：**视频预测和动作预测之间的耦合机制是怎样的？**

直觉上，联合训练视频和动作应该让两者互相受益——视频理解帮助动作预测，动作信号帮助视频生成。但具体是**什么机制**在起作用？是推理时视频输出的质量决定了动作的精度，还是训练时共享权重的梯度传递在起作用？这个问题的答案直接决定了两个应用方向的可行性：

1. 能否跳过视频生成来加速推理？（如果推理时弱耦合 → 可以）
2. 能否通过优化视频生成来间接改善动作？（如果训练时强耦合 → 可以）

## 二、先导实验：DreamZero 上的 7 组验证
