...
**核心论点：COSMOS 3 证明了训练应该在一起（共享参数 = 知识迁移）。FastWAM 证明了推理应该分开（Expert 分离 = 性能 + 无有害干扰）。我们的目标架构两者兼得：训练时共享 Attention 实现知识迁移，推理时 KV cache 分离实现多频率执行。**
...
> **核心论点**：COSMOS 3 证明了训练应该在一起——LLM + Video + Action 在共享参数下联合训练，AR 阶段学到的语义理解、物理常识、视觉表征通过共享权重直接迁移到 Action 生成。FastWAM 证明了推理应该分开——Expert 分离架构下，各模态通过 KV cache 独立执行，获得多频率性能和零有害干扰。我们的目标架构结合两者：训练时共享 Attention 实现知识迁移，推理时 KV cache 分离实现多频率执行。
...
> COSMOS 3 证明了训练应该在一起（共享参数 = 语义/物理/视觉知识迁移到 Action）。FastWAM 证明了推理应该分开（Expert 分离 = 多频率 + 无有害干扰 + 4× 加速）。我们的目标架构两者兼得：训练时共享 Attention 实现知识迁移，推理时 KV cache 分离实现多频率执行。这是三 Expert 分离架构的出发点。
